            ...
            if ($user) {
                if (($Password==$user["password"])&&($email ==$user["email"])) {
                   if($email == 'bts_nitc@nitc.ac.in'){//enter username and password for driver
                     session_start();
                     $vsql="INSERT into login_details(email,status,role) values('$email','Login Successfull','Driver');";
                        $res=mysqli_query($conn,$vsql);
                      $_SESSION["adminLH"] = "yes";
                       header("Location: index.php");//users header location
                     
                       die();
                   }elseif($email == 'bts_mhb1@nitc.ac.in'){
                       $vsql="INSERT into login_details(email,status,role) values('$email','Login Successfull','Driver');";
                        $res=mysqli_query($conn,$vsql);
                     session_start();
                      $_SESSION["adminMHB1"] = "yes";
                       header("Location: index2.php");//drivers header location
                       die();
                     }
                     elseif($email == 'bts_mhb2@nitc.ac.in'){ 
                         $vsql="INSERT into login_details(email,status,role) values('$email','Login Successfull','Driver');";
                        $res=mysqli_query($conn,$vsql);
                     session_start();
                      $_SESSION["adminMHB2"] = "yes";
                       header("Location: index3.php");//drivers header location
                       die();
                     }elseif($email == 'bts_soms@nitc.ac.in'){  
                         $vsql="INSERT into login_details(email,status,role) values('$email','Login Successfull','Driver');";
                        $res=mysqli_query($conn,$vsql);
                     session_start();
                      $_SESSION["adminSOMS"] = "yes";
                       header("Location: index4.php");//drivers header location
                       die();
                     }else{  
                         $vsql="INSERT into login_details(email,status,role) values('$email','Login Successfull','Passenger');";
                        $res=mysqli_query($conn,$vsql);
                     session_start();
                      $_SESSION["user"] = "yes";
